...
export function useChatEngine({ snapshot, refreshSnapshot, currentUser, toast }) {
  ...
  const addConfirmedSlot = useCallback(async (emp, slots) => {
    try {
      ...
      addMsg({ role: "assistant", kind: "text", content: `Confirmed timeslot added for ${emp.name} on ${formatScheduledDate(slots.date)}, ${formatTimeRange(slots.time.start, slots.time.end)}.\n\nNow try scheduling again — the slot is available!` });
  ...
  // B) Raise pending timeslot request — REMOVED (not needed anymore)
  ...
  const confirmSchedule = useCallback(async (msg) => {
    ...
    try {
      ...
      // First message: Green success box
      addMsg({
        role: "assistant", kind: "success", interview_id: id,
        content: `${msg.candidate.candidate.name} with ${msg.interviewer.name}\n${formatScheduledDate(msg.slots.date)}, ${formatTimeRange(msg.slots.time.start, msg.slots.time.end)}, ${msg.slots.mode || "Video"}`
      });
      
      // Second message: Next suggestion in separate box
      addMsg({
